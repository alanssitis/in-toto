...
<Purpose>
  Generate a layout file from a file.
"""
...
from in_toto.layout_pydantic import Config
...
def _parse_config(config):
  ...
  raise ValueError(f"Could not guess the config type of '{config.name}'!")
...
def _parse_expires(expires):
  ...
  if isinstance(expires, datetime) or isinstance(expires, date):
    return expires.isoformat()

  if match := re.fullmatch(r"^(\d+)(s|m|h|d|w)$", expires):
  ...
  raise ValueError(f"Could not parse layout expiration '{expires}'")
...
def main():
  """Generate an in-toto layout from a yaml or toml file.
  """
  args = _parse_args()
  ...
  if signer_key is None:
    raise ValueError('Do not have a signing key')
